...
def get_gamma_version() -> str:
    """
    Get the version of GAMMA being used by the `ga_sar_workflow` workflow.
...
def create_versioned_gamma_proxy(gamma_ver: str, base_wrapper: object, exception_type: Type[Exception]) -> PyGammaProxy:
    ...
    This is used so `ga_sar_workflow` can be written for one specific GAMMA version without having to
    ...
    return cast(GammaProxy, wrapper_module.GammaProxy(exception_type, base_wrapper))


def create_gamma_proxy(exception_type: Type[Exception]) -> GammaProxy:
    """

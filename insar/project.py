...
class ProcConfig:
    ...
    def __init__(self, outdir, **kwargs):
        ...
        if self.ref_primary_scene.lower() == "auto" and outdir:
            # Read computed primary scene and use it
            with open(pathlib.Path(outdir) / self.list_dir / 'primary_ref_scene', 'r') as ref_scene_file:
                auto_primary_scene = ref_scene_file.readline().strip()

            self.ref_primary_scene = auto_primary_scene

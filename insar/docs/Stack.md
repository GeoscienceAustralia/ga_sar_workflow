...
This document provides an overview of what a "stack" is in the `ga_sar_workflow` project.
...
| Processing settings | A processing configuration file with a set of properties which define 'how' the ga_sar_workflow project processes the stack's scenes. |
...
Lastly the main output product of `ga_sar_workflow` is the interferometry itself, which is produced for the baseline determined by the stack - most scenes will take part in multiple baseline (this can be controlled by the `MIN_CONNECT` and `MAX_CONNECT` .proc settings).
...
For `ga_sar_workflow` developers, the paths specified in this document are defined in the `insar.paths` modules - and must match the specifications in this document.  If either changes, the other should be updated to reflect the changes where appropriate.
